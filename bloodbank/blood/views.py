...
@login_required(login_url='adminlogin')
def cost_of_blood(request):
    cost_form = forms.CostOfBloodStockForm(request.POST)
    if request.method == "POST":
        if cost_form.is_valid():
            stock_form = cost_form.save(commit=False)
            stock_form.blood_stock_price = request.POST['cost']
            stock_form.save()
            return render(request,'blood/admin_blood.html',)
        else:
            cost_form = forms.CostOfBloodStockForm()
    return render(request,'blood/admin_blood.html',{'cost_form':cost_form})
